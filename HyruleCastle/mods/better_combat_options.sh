...
"
   read OPT
   if [ $OPT == Attack ] || [ $OPT == 1 ] || [ $OPT == attack ] || [ $OPT == ATTACK ]
...
"
   ...
   if [ $OPT == Heal ] || [ $OPT == 2 ] || [ $OPT == heal ] || [ $OPT == HEAL ]
   then HP_P=$(($HP_P-$STR_E+$HP_Pmax/2))
        if [ $HP_P -gt $HP_Pmax ] ; then HP_P=$HP_Pmax ; fi
	echo -e "$P : $HP_P / $HP_Pmax "
	echo "$E : $HP_E / $HP_Emax
"
   fi

   if [ $OPT == Protect ] || [ $OPT == 3 ] || [ $OPT == protect ] || [ $OPT == PROTECT ]
      then  HP_P=$(($HP_P-$STR_E/2))
	echo "$P : $HP_P / $HP_Pmax"
        echo "$E : $HP_E / $HP_Emax
"
   fi

   if [ $OPT == Escape ] || [ $OPT == 4 ] || [ $OPT == escape ] || [ $OPT == ESCAPE ]
   then echo "
Are you sure? Escaping will reset you back to the beginning...

                       YES   NO  ?
" ; read ESC ; if [ $ESC == YES ] ; then exit ; fi
	if [ $ESC == NO ] ; then echo "
